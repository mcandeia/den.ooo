...
export type VersionCache = {
  ...
  get: <T extends string | string[] = never>(
    key: string,
  ...
  fetch = async (req: Request): Promise<Response> => {
    ...
    if (pieces[1].includes("@")) {
      if (this.options.aliases[pieces[1].split("@")[0]]) {
        pieces[1] = this.options.aliases[pieces[1].split("@")[0]] + "@" +
          pieces[1].split("@")[1];
    ...
    if (data.version === null) {
      ...
      if (wasAlias) {
        ...
        return Response.redirect(
          `${isDev() ? "http" : "https"}://${this.domain}${
            originalPathname.join("/")
          }`,
          307,
        );
      } else {
        return Response.redirect(
          resolver.getRedirectUrl(this, data as ModuleData),
          307,
        );
      }
    } else {
